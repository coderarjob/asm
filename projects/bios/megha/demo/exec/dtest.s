...
readfiledata:
	; set the requested size of the file = file size 
	mov ax, [filesize]
	mov [fileremsize], ax		
	mov [filereqsize], ax
.repeat:
...
.readFileEnd:
	...
	; before transfering to the program we change the 
	; segment registers
	push ds
	push es
	push gs
...
; the following is used to hold the output buffer address
osegoffset	equ 0x0
osegment	equ 0x2000
