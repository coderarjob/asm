...
bootloader_start:
	...
	mov al,03h		; 80 columns and 25 lines text mode
	int 10h
...
drawpallet:
	; display the 16 color pallet of the text mode
	mov bx,00		; row
...
drawpixel:
	...
	shl bx,1
	shl cx,1
	; mem = row * 80 + column => bx = bx << 6 + bx << 4 + dx
	mov dx,bx
	shl bx,6
	shl dx,4
	add bx,dx
